...
public class Main {
    ...
    public static boolean IsKnightSafe(boolean[][] chess, int i, int j) {
        ...
        for(int row = i, col = j; row >= 0 && col < n; row -= 1, col += 2){
            if(chess[row][col]){
                return false;
            }
        }
        for(int row = i, col = j; row >= 0 && col < n; row -= 2, col += 1){
            if(chess[row][col]){
                return false;
            }
        }
        for(int row = i, col = j; row >= 0 && col >= 0; row -= 2, col -= 1){
            if(chess[row][col]){
                return false;
            }
        }
        for(int row = i, col = j; row >= 0 && col >= 0; row -= 1, col -= 2){
            if(chess[row][col]){
                return false;
            }
        }
