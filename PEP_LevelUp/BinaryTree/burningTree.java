...
public class Main {
    ...
    private static void burnTree(TreeNode root, TreeNode blockNode, int time) {

        if (root == null || root == blockNode) return; // can't burn
    ...
    private static int burningTreeHelper(TreeNode root, int fireNode) {
        ...
        int leftTime = burningTreeHelper(root.left, fireNode);
        if (leftTime != -1) {
            burnTree(root, root.left, leftTime);
            return leftTime + 1;
        }

        int rightTime = burningTreeHelper(root.right, fireNode);
        if (rightTime != -1) {
