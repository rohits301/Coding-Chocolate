...
public class Main {
    ...
    private static class BSTPair {
        ...
        int max;
        boolean isBST;
        Node root; // my largestBST root
        int size; // my largestBST size
    }
    ...
    private static BSTPair largestBST(Node node) {
        ...
        // isBST-> treeBST=> nodeBST and lpBST and rpBST

        mp.isBST = (node.data >= lp.max && node.data <= rp.min) && 
                    lp.isBST && 
                    rp.isBST;
        mp.min = Math.min(node.data, Math.min(lp.min, rp.min));
        mp.max = Math.max(node.data, Math.max(lp.max, rp.max));

        // extra code for LargestBST
        if (mp.isBST) {
            mp.root = node;
